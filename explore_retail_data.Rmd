---
title: "Exploring Online Retail II Dataset"
...
`invoice_date` is actually a time stamp.
...
data_tbl <- data_tbl %>%
  ...
  mutate(
    missing_cust_id = are_na(customer_id),
    cancelled_tnx   = str_detect(invoice_id, "^C"),
...
for(plot_varname in logical_vars) {
  ...
  explore_plot <- ggplot(plot_tbl) +
    ...
    ylab("Count") +
    scale_y_continuous(labels = comma) +
    ggtitle(str_c(facet_varname, '-Faceted Barplots for Variable: ', plot_varname)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
...
for(plot_varname in numeric_vars) {
  ...
  explore_plot <- ggplot(plot_tbl) +
    ...
    scale_y_continuous(labels = comma) +
    ggtitle(str_c(facet_varname, '-Faceted Histogram for Variable: ', plot_varname)) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
